...
var pJS = function (tag_id, params) {
	...
	this.pJS = {
		...
		particles: {
			...
			shape: {
				...
				image: {
					src: "img/popcorns/popcorn_1.png",
					width: 100,
					height: 100,
				},
			},
			...
			life: {
				...
				endOfLifeBehaviour: {
					type: "drag", // drag - none
					dragToPosition: {
						// Move with the physics engine to the location
						radius: 20,
						x: 0,
						y: 0,
					},
					onDeath: "func", // func - none
					onDeathFunc: undefined, // What to do on death
				},
			},
			move: {
				enable: true,
				...
				physicsEngine: {
					...
					gravity: 0.2,
					acceleration: 1.01,
					weight: [2, 4], // Randomize weight between
					maxSpeed: 15,
					bounceVelocity: 0,
					gravityDirection: "bottom",
					dragToPoint: {
						enable: true,
						enableAfterDuration: [100, 140],
						x: 200,
						y: 400,
						force: 0.4,
						radius: 20,
						onIntersect: "func",
						toPerformInline: function (p, index) {
							pJS.fn.modes.removeParticle(index);

							if (this.toPerform !== undefined) {
								this.toPerform();
							}
						},
						toPerform: function () {
							popcornContainer.AddPopcorns(10);
						},
					},
				},
				attract: {
	...
	pJS.fn.add = function (amount, x, y, lifetime) {
		pJS.fn.particlesCreate(amount, x, y, lifetime);
	};

	pJS.fn.particle = function (
		color,
		opacity,
		position,
		ignoreOverlap,
		...
		if (pJS.particles.size.random) {
			if (pJS.particles.size.randomSize.length > 1) {
				this.radius =
					Math.random() *
						(pJS.particles.size.randomSize[1] -
							pJS.particles.size.randomSize[0]) +
					pJS.particles.size.randomSize[0];
			} else {
		...
		if (pJS.particles.move.bounce && !ignoreOverlap) {
		...
		/* physics - weight and such */
		if (pJS.particles.move.physicsEngine.enable) {
			this.weight =
				Math.random() *
					(pJS.particles.move.physicsEngine.weight[1] -
						pJS.particles.move.physicsEngine.weight[0]) +
				pJS.particles.move.physicsEngine.weight[0];
			this.velocity = {
		...
		this.activateDragDuration =
			lifetime !== 0
				? 0
				: pJS.particles.move.physicsEngine.dragToPoint.enable
				? Math.random() *
						(pJS.particles.move.physicsEngine.dragToPoint
							.enableAfterDuration[1] -
							pJS.particles.move.physicsEngine.dragToPoint
								.enableAfterDuration[0]) +
				  pJS.particles.move.physicsEngine.dragToPoint.enableAfterDuration[0]
				: 0;
		...
		for (var i = 0; i < pJS.particles.array.length; i++) {
			...
			if (pJS.particles.move.enable) {
				...
				if (pJS.particles.move.physicsEngine.enable) {
					...
					var force = pJS.particles.move.physicsEngine.dragToPoint.force;

					var dragToPoint =
						p.activateDragDuration === 0
							? false
							: pJS.particles.move.physicsEngine.dragToPoint.enable
							? p.lifeDuration >= p.activateDragDuration
							: false;

					var attractionForceY = dragToPoint ? axis.y * (p.weight * force) : 0;
					...
					if (Math.abs(p.velocity.y) > gravity) {
						p.y += p.velocity.y;
					}
					...
					if (dragToPoint) {
						var radius = pJS.particles.move.physicsEngine.dragToPoint.radius;
						var x = pJS.particles.move.physicsEngine.dragToPoint.x;
						var y = pJS.particles.move.physicsEngine.dragToPoint.y;
						...
						if (
							...
							pJS.particles.move.physicsEngine.dragToPoint.toPerformInline(
								p,
								i
							);
						}
			...
			/* Life duration */
			if (p.lifeDuration > p.lifetime && p.lifetime > 0) {
				pJS.fn.modes.removeParticle(i);
			}
		}
	};
	...
	pJS.fn.modes.removeParticle = function (index) {
		pJS.particles.array.splice(index, 1);
		if (!pJS.particles.move.enable) {
			pJS.fn.particlesDraw();
		}
	};
...
window.particlesJS.load = function (tag_id, callback) {
	/* load json config */
	window.particlesJS(tag_id, undefined);
