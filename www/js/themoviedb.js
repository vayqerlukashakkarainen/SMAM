...
theMovieDb.common = {
	api_key: "45bf423c78a26bc23a95da412aa87a2e",
	base_uri: "https://api.themoviedb.org/3/",
	content_uri: "https://www.themoviedb.org/",
		...
		xhr.onload = function (e) {
			if (xhr.readyState === 4) {
				if (xhr.status === status) {
					...
					error(`${xhr.responseText} code: ${xhr.status}`);
				}
			} else {
				error(`${xhr.responseText} code: ${xhr.status}`);
			}
...
theMovieDb.providers = {
	getAvailableRegions: function (options, success, error) {
		"use strict";

		theMovieDb.common.validateRequired(arguments, 3, options, "", true);
		...
		theMovieDb.common.client(
			{
				url:
					"watch/providers/regions" + theMovieDb.common.generateQuery(options),
			},
			success,
			error
		);
	},
	getMovieProviders: function (options, success, error) {
		"use strict";
		...
		theMovieDb.common.client(
			{
				url: "watch/providers/movie" + theMovieDb.common.generateQuery(options),
			},
			success,
			error
		);
	},
	getTvProviders: function (options, success, error) {
		"use strict";
		...
		theMovieDb.common.client(
			{
				url: "watch/providers/tv" + theMovieDb.common.generateQuery(options),
			},
			success,
			error
		);
...
theMovieDb.movies = {
	...
	getWatchProviders: function (options, success, error) {
		"use strict";
